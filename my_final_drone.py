        ...
        if self.communication:
            ...
            for msg in messages:
                try:
                    msg = msg[1]
                    other_pos = msg[2]
                    if np.sqrt(np.dot(pos - other_pos, pos - other_pos)) < 200 and msg[0] != self.identifier and msg[3] == self.Activity.GOING_BACK:
                        return False
        ...
        if self.state is self.Activity.INITIALISATION:
            ...
            if self.path == []:
                ...
                try:
                    ...
                    self.state = self.Activity.BACKUP_RIGHT if self.Type is self.Type.LEADER_RIGHT else self.Activity.BACKUP_LEFT
            else:
